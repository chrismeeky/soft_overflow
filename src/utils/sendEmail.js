...
class SendEmail {
  ...
  */
  ...
  /**
  * @param {object} responder - An object
  *  @param {object} recipient - An object
  * containing details required to send notifications
  * @returns {boolean} specifies if the email was sent successfully
  */
  static sendEmailNotification(responder, recipient) {
    const details = {
      email: recipient.email,
      subject: 'New comment - Softoverflow',
      html: `'<div style="width: 90%; margin: 5em auto;
        box-shadow: 0 0 10px rgba(0,0,0,.9);">
        <div>
          <div>
            ...
            <h1 style="text-align: center">Hi! ${recipient.firstName}</h1>
          </div>
          <div style=" padding: 0px 20px 20px 20px">
            <div>
              <p>You have a new <a href="${baseUrl}"/> comment from user: <strong>${responder.username}</strong> that you might want to checkout</p>
              <button style="color: white; background-color: #1AC124;
                border: none; border-radius: 10px; text-align: center;
                padding: 10px;">
                <a  href="${baseUrl}"
                  style="text-decoration: none;
                  color: white;">View Comment</a></button>
            </div>
            <div>
              <h3 style="text-align: center">Thank you</h3>
              <h3 style="text-align: center">
              Please do not reply directly, this is an autogenerated email.</h3>
            </div>
          </div>
        </div>
      </div>`,
